...
class Library extends Contract{
    ...
    async accessrequest(ctx, rollno, operation, filename){
        const userAsBytes = await ctx.stub.invokeChaincode("institute",["invoke",rollno],"mychannel");
        if(!userAsBytes||userAsBytes.length==0){
            throw new Error(`${rollno} does not exist`);
        }
        const user = JSON.parse(userAsBytes.payload.toString('utf-8'));
